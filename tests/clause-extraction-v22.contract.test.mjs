...
const finalization = readFileSync(new URL('skills/clause-extraction/references/e10-finalization-and-handoff.md', root), 'utf8')
const currentContract = [skill, admission, finalization].join('\n')
...
const skillScaffold = readFileSync(new URL('skills/clause-extraction/references/clause-extraction-artifact-v22-authoring-scaffold.yaml', root), 'utf8')
...
test('uses the byte-pinned enabled-Skill schema and returns only to the synchronous Lead caller', () => {
  assert.equal(createHash('sha256').update(bytes).digest('hex'), 'fc5355dfc349bde3ff52e0b05171010f7b6d8db0d5d3811576baf02ae7090d3d')
  assert.deepEqual(skillSchemaBytes, bytes)
  assert.equal(v22Template, skillScaffold)
  ...
  assert.match(currentContract, /512 KiB, 8,000 nodes and depth 64/)
  assert.match(currentContract, /不得把返回 hash、模型自述或 `valid` 字段写进业务产物/)
  assert.match(currentContract, /Lead 必须仍按 O2 同调用 Compose 和既有 RC\/HG 验收/)
  assert.match(currentContract, /members\/clause-extractor\/<入站 case_id>\/<本次 extraction_id>\/artifact\/<extraction_id>\.extraction\.yaml/)
  assert.match(currentContract, /不得从 task、文件名或旧摘要猜 case\/member binding/)
  assert.match(currentContract, /独立运行使用本 Agent 已确认 workspace/)
})
test('v2.2 permits a truthful empty ambiguity list and keeps the authoring scaffold non-ready', () => {
  const validate = new Ajv({ strict: false }).compile(schema)
  const clean = parse('valid-ready-v22-docx.yaml')
  clean.clause_extraction.ambiguities = []
  assert.equal(validate(clean), true, JSON.stringify(validate.errors))
  const scaffold = YAML.parse(skillScaffold)
  assert.equal(scaffold.clause_extraction.lifecycle.state, 'partial')
  assert.equal(scaffold.clause_extraction.lifecycle.handoff, null)
  assert.equal(validate(scaffold), false, 'the incremental authoring scaffold is intentionally incomplete')
})
